...
import com.zlo.inequa.domain.model.RationalExpression

class RationalExpressionParser {

    fun parse(rawExpression: String): RationalExpression {
    ...
    private fun parseExpression(stream: TokenStream): RationalExpression {
    ...
    private fun parseTerm(stream: TokenStream): RationalExpression {
        ...
        while (true) {
            value = when (stream.peek()) {
                ...
                Token.Divide -> {
                    stream.consume()
                    value.divide(parseSignedPower(stream))
                }
    ...
    private fun parseSignedPower(stream: TokenStream): RationalExpression {
        return when (stream.peek()) {
            ...
            Token.Minus -> {
                stream.consume()
                RationalExpression.constant(-1.0).multiply(parseSignedPower(stream))
            }
    ...
    private fun parsePower(stream: TokenStream): RationalExpression {
    ...
    private fun parsePrimary(stream: TokenStream): RationalExpression {
        return when (val token = stream.peek()) {
            is Token.Number -> {
                stream.consume()
                RationalExpression.constant(token.value)
            }

            Token.Variable -> {
                stream.consume()
                RationalExpression.polynomial(PolynomialExpression.X)
            }
